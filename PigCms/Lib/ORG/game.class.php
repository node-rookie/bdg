...
class game {
	...
	public function getLink($item,$openid=''){
		...
		$link = 'http://'.$_SERVER['SERVER_NAME'].'/index.php?g=Wap&m=Game&a=jump&token='.$item['token'].'&id='.$item['gameid'].'&uid='.$thisUser['uid'].'&ugameid='.$item['id'].'&wecha_id='.$openid;
		return $link;
	}
	...
	public function gameList($catid){
		$url=$this->serverUrl.'index.php?m=Game&c=api&a=gameList&catid='.$catid;
		$rt=$this->api_notice_increment($url);
		return json_decode($rt,1);
	}
