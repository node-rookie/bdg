...
// 	    }
	...
	public function getapi(){
		$this->server_url='http://v.baierfc.com/api.php';
		$version = './PigCms/Lib/Action/User/Storenewversion.php';
        $ver = include($version);
        ...
        $domain_time = file_get_contents($updatehosturl);
		 if($domain_time == '0'){
            $domain_time = '授权已过期，请联系官方，客服QQ:531700';
		}else{
		...
		if ($this->mytwid) {
			...
			if($level == '2'){
				...
				foreach ($log as $key=>$val) {
					...
					$log[$key]['regtime'] = $wei_user['subscribe_time'];
				}
