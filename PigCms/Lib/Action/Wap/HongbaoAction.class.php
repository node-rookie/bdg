...
class HongbaoAction extends WapAction
{
	...
	public function _initialize()
	{
		...
		$this->share_key = $this->_get('share_key', 'trim');
		$this->now = time();
		$this->action = M('hongbao')->where(array('id' => $this->action_id, 'token' => $this->token, 'status' => '1'))->find();
		$this->total_grabber = M('hongbao_grabber')->where('isgrabbed != 0 and hongbao_id = ' . $this->action_id)->count();
	...
	{
		if (empty($this->action_id) || empty($this->action)) {
			$this->error('活动不存在！');
			exit();
		}
		...
		if (($this->action['sharetimes'] <= $share_times) && ($this->my_packets['isgrabbed'] == 1)) {
		...
		if ($this->wecha_id) {
			$data = array('hongbao_id' => (int) $this->action_id, 'money' => 0, 'grabber_nickname' => !empty($this->fans['wechaname']) ? $this->fans['wechaname'] : 'anonymous', 'grabber_headimgurl' => !empty($this->fans['portrait']) ? $this->fans['portrait'] : $this->siteUrl . 'tpl/User/default/common/images/portrait.jpg', 'grabber_shareid' => $this->get_key(32), 'grabber_wechaid' => $this->wecha_id, 'grabber_sex' => !empty($this->fans['sex']) ? $this->fans['sex'] : 0, 'grabber_tel' => !empty($this->fans['tel']) ? $this->fans['tel'] : '111111', 'grabber_qq' => !empty($this->fans['qq']) ? $this->fans['qq'] : '111111', 'grabber_address' => !empty($this->fans['address']) ? $this->fans['address'] : 'qwert', 'grabber_province' => !empty($this->fans['province']) ? $this->fans['province'] : 'ah', 'grabber_city' => !empty($this->fans['city']) ? $this->fans['city'] : 'hf', 'share_money' => 0, 'grabber_time' => 0, 'isgrabbed' => 0, 'token' => $this->token);
		...
		$this->assign('total', $this->total_grabber);
	...
	public function grab_packet()
	{
		...
		if (($this->action['need_follow'] == 1) && ($this->isSubscribe() == false)) {
			$this->error('请先关注然后再抢红包');
			exit();
		}

		if (($this->action['need_phone'] == 1) && empty($this->fans)) {
			$this->error('请先注册然后抢红包');
			exit();
		}

		if (time() < $this->action['start_time']) {
		...
		$rand_money = $this->rand_money($this->action['min_money'], $this->action['max_money']);

		if ($this->action['total_money'] <= $rand_money) {
			$this->error('红包已经被抢完了!');
			exit();
		}
	...
	public function find_helper()
	{
		...
		$this->notice();
		$grabber = M('hongbao_grabber')->where(array('grabber_shareid' => $this->share_key))->find();
		$share = M('hongbao_share')->where(array('hongbao_id' => $this->action_id, 'share_key' => $this->share_key, 'share_wechaid' => $this->wecha_id))->find();
		if ($this->share_key && $grabber && ($grabber['grabber_wechaid'] != $this->wecha_id) && $this->wecha_id) {
			if (empty($share) || ($share['is_opened'] == 0)) {
				$share = array('hongbao_id' => $this->action_id, 'share_key' => $this->share_key, 'rand_money' => 0, 'share_nickname' => !empty($this->fans['wechaname']) ? $this->fans['wechaname'] : 'as', 'share_pic' => !empty($this->fans['portrait']) ? $this->fans['portrait'] : $this->siteUrl . 'tpl/User/default/common/images/portrait.jpg', 'share_time' => $this->now, 'is_opened' => 1, 'share_wechaid' => $this->wecha_id);
		...
		$this->assign('action_id', $this->action_id);
		$this->assign('token', $this->token);
		$this->assign('share_key', $this->share_key);
		...
		if ($this->action_id && $this->token) {
			...
			if (!empty($share)) {
				if ($share['is_opened'] == 1) {
					$pack = M('hongbao_grabber')->where(array('hongbao_id' => $this->action_id, 'token' => $this->token, 'grabber_shareid' => $this->share_key))->find();

					if ($pack['isgrabbed'] == 2) {
						exit('红包已经被领取');
					}

					$where_share = array('hongbao_id' => $this->action_id, 'share_key' => $this->share_key, 'is_opened' => 2);
					$my_share_times = M('hongbao_share')->where($where_share)->count();

					if ($this->action['sharetimes'] <= $my_share_times) {
						exit('合体次数已到,你也去抢把');
					}
					...
					$u_share = M('hongbao_share')->where(array('hongbao_id' => $this->action_id, 'token' => $this->token, 'share_key' => $this->share_key, 'share_wechaid' => $this->wecha_id))->save(array('add_money' => $rand_money, 'is_opened' => 2));
					$data = array();
					$data['money'] = array('exp', 'money+' . $rand_money);
					$u_grabber = M('hongbao_grabber')->where(array('hongbao_id' => $this->action_id, 'token' => $this->token, 'grabber_shareid' => $this->share_key))->save($data);
					$hb_d = array();
					$hb_d['total_money'] = array('exp', 'total_money-' . $rand_money);
	...
	public function notice()
	{
		if (($this->action['need_follow'] == 1) && ($this->isSubscribe() == false)) {
			...
			if (($this->action['need_phone'] == 1) && empty($this->fans['tel'])) {
				$this->memberNotice();
			}
			else {
				if ((strpos($this->action['remind_link'], '{siteUrl}') !== false) || (strpos($this->action['remind_link'], '{wechat_id}') !== false)) {
					$link = str_replace(array('{siteUrl}', '{wechat_id}'), array(C('site_url'), $this->wecha_id), $this->action['remind_link']);
				}
				else {
					$link = $this->action['remind_link'];
				}

				$publicnum = M('home')->where(array('token' => $this->token))->find();
				$this->assign('remind_word', $this->action['remind_word']);
