...
class UserinfoAction extends WapAction
{
	...
	public function index()
	{
		...
		if ($conf == NULL) {
			$conf = array('wechaname' => 1, 'tel' => 1, 'truename' => 1, 'qq' => 1, 'paypass' => 1, 'portrait' => 1, 'sex' => 1, 'bornyear' => 1, 'bornmonth' => 1, 'bornday' => 1);
		}
		...
		if (IS_POST) {
			...
			if ($cardid == 0) {
				...
				$userInfos = $userinfo_db->where(array('token' => $this->_get('token'), 'wecha_id' => $this->wecha_id))->select();
				$userScore = 0;
				...
				if (!$card) {
					...
					if ((intval($thisCard['miniscore']) == 0) || (intval($thisCard['minscore']) < $userScore)) {
						...
						if (empty($is_card)) {
							...
							$uinfo = M('Userinfo')->where(array('token' => $this->token, 'wecha_id' => $this->wecha_id))->find();
							$now = time();
							$gwhere = array(
								'token'   => $this->token,
								'cardid'  => $cardid,
