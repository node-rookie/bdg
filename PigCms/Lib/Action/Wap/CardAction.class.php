...
/*              $owhere         = array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'is_open'=>'1','cardid'=>$thisCard['id'],'start'=>array('lt',time()),'end'=>array('gt',time()));
    ...
    public function integral(){
        ...
        $thisCard=$this->_thisCard();
        $this->assign('thisCard',$thisCard);
        $is_use     = strval($this->_get('is_use','intval')?$this->_get('is_use','intval'):'0'); //自行增加 strval 
        $now=time();
    ...
    public function my_coupon(){
        ...
        $where  = array('token'=>$this->token,'attr'=>'0','statdate'=>array('lt',$now),'enddate'=>array('gt',$now),'is_check'=>1,'is_delete'=>0);
        if($type == 1){
            $where['type'] = '1';
        }else if($type == 2){
            $where['type'] = '0';
    ...
    public function coupon(){
        ...
        $is_use     = strval($this->_get('is_use','intval')?$this->_get('is_use','intval'):'0');//自行增加strval 
        $now=time();
