...
class WeixinAction extends BaseAction{
	...
	public function new_pay(){
		...
		$unifiedOrder->setParameter("openid",$_GET['wecha_id']);//商品描述
		...
		$unifiedOrder->setParameter("notify_url",C('site_url').'/wxpay/'.$noticeFileName);//通知地址 
	...
	public function new_return_url (){
		...
		if(intval($_GET['total_fee']) && !intval($_GET['trade_state'])) {
			...
			if (class_exists('ThirdPay'.$_GET['from'])){
				...
				$payobj->index($out_trade_no,'weixin',$_GET['transaction_id']);
			}
			// $mode_name = $_GET['from'].'Model';
			// $import_return = import('@.Model.'.$mode_name);
			// if($import_return){
				// $mode_obj = new $mode_name;
				// if(method_exists($mode_obj,'after_pay_auto')){
					// $mode_obj->after_pay_auto($out_trade_no,'weixin',$_GET['transaction_id']);
				// }
			// }
