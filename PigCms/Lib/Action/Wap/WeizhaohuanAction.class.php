...
class WeizhaohuanAction extends WapAction{
	...
	public function rank(){
		...
		$urank 	= array();
		$uinfo 	= M('Weizhaohuan_user')->where(array('token'=>$this->token,'pid'=>$id,'id'=>$uid))->field('wecha_id,share_count')->find();
		$urank['count'] 	= (M('Weizhaohuan_user')->where(array('token'=>$this->token,'pid'=>$id,'is_real'=>1,'share_count'=>array('gt',$uinfo['share_count'])))->count())+1;
		$urank['username'] 	= M('Userinfo')->where(array('token'=>$this->token,'wecha_id'=>$uinfo['wecha_id']))->getField('wechaname');
		
		
		$allcount 	= M('Weizhaohuan_user')->where(array('token'=>$this->token,'pid'=>$id))->count();
	...
	public function add_puser($wecha_id,$is_real=1,$id){
		...
		$user = M('Weizhaohuan_user')->where(array('token'=>$this->token,'wecha_id'=>$wecha_id,'is_real'=>$is_real))->find();
		if(empty($user)){
