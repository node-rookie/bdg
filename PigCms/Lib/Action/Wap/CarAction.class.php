...
class CarAction extends WapAction{
    ...
    public function index(){
        ...
        $info=D('Carcat')->where(array('token'=>$token,'is_show'=>'1'))->field('name,img,url')->order('sort desc')->select();
        foreach ($info as $key=>$value){
        	$info[$key]['url']  = $this->getLink($value['url']);
    ...
    public function maps(){
        ...
        $reser =  M('Reservation')->where($where)->find();

        if (!$this->isamap){
            $this->display();
        }else {         
            $link=$this->amap->getPointMapLink($reser['lng'],$reser['lat'],$reser['title']);
