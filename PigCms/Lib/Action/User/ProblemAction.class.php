...
class ProblemAction extends UserAction{
			...
			if($set_db->create()){
				...
				if(empty($problem_info)){
					...
					if($id){
	                	...
	                	$this->handleKeyword($id,'Problem',$this->_post('keyword','trim'));
					}
				...
				//修改
					 ...
					 */
						$this->handleKeyword($this->_post('id','intval'),'Problem',$this->_post('keyword','trim'));
					}
	...
	public function show_question_log(){
		...
		$log 		= M('problem_question_log')->where($where)->order('score desc,add_time asc')->limit($Page->firstRow.','.$Page->listRows)->select();
