...
class MicrstoreAction extends UserAction
{
	...
	public function api()
	{
		...
		$data = array('token' => $this->token, 'site_url' => $this->siteUrl, 'payment_url' => $this->siteUrl . U('Wap/Micrstore/pay'), 'login_url' => $this->siteUrl . U('Wap/Micrstore/login'), 'notify_url' => $this->siteUrl . U('Wap/Micrstore/notify'), 'timestamp' => $wxuser['createtime'], 'wxname' => $wxuser['wxname'], 'server_key' => C('server_key'), 'app_id' => 1);
		$sort_data = $data;
	...
	public function withdraw()
	{
		...
		$list = M('microsoft_withdraw')->where(array('token' => $this->token, 'is_show' => 1))->limit($page->firstRow . ',' . $page->listRows)->select();
