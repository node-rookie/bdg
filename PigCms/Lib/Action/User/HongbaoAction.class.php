...
class HongbaoAction extends UserAction
{
	...
	public function get_packet_list()
	{
		...
		$total = M('hongbao_grabber')->where(array('token' => $this->token, 'grabber_id' => $id))->count();
		$page = new Page($total, 15);
		$list = M('hongbao_grabber')->query('select h.action_name,g.grabber_id,g.grabber_nickname,g.money,g.isgrabbed from ' . C('DB_PREFIX') . 'hongbao_grabber as g,' . C('DB_PREFIX') . 'hongbao as h where h.id = g.hongbao_id AND g.token = \'' . $this->token . '\' AND h.id = ' . $id . ' order by id desc limit ' . $page->firstRow . ',' . $page->listRows);
		$this->assign('list', $list);
		$this->assign('page', $page->show());
		$this->display();
	}
