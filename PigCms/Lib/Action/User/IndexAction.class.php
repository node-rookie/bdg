...
class IndexAction extends UserAction{
	...
	public function edit(){
		...
		if(in_array('liaotian',$queryname)){
			$res['wx_liaotian'] = '1';//自行修改 必须为字符型
		}else{
			$res['wx_liaotian'] = '0';//自行修改 必须为字符型
		}
		...
		//dump($res);
		//die;
		if(($res['appid'] != '' || $res['appsecret'] != '') && ($res['winxintype'] == 3 || $res['winxintype'] == 4) && $res['type'] != 1){
			$appid = $res['appid'];
