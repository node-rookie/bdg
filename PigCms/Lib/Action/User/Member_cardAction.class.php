...
//删除记录
	...
	public function donate_set(){
		...
		if(IS_POST){
			$min_price 		= $this->_post('min_price','intval');
			$max_price 		= $this->_post('max_price','intval');
			$donate_price 	= $this->_post('donate_price','intval');
	...
	public function find_coupons(){
		...
		$coupons= M('Member_card_coupon')->where(array('token'=>$this->token,'id'=>$info['coupon_id']))->field('title,statdate,enddate,is_weixin,brand_name,logourl,is_weixin,info')->find();
	...
	public function coupons_record(){
        ...
        $list 	= M('Member_card_coupon_record')->alias('record')
	            ...
	            ->join('LEFT JOIN '.C('DB_PREFIX').'userinfo user ON record.wecha_id=user.wecha_id')
            ->where($where)
            ->field('record.id,record.add_time,record.staff_id,record.company_id,record.cancel_code,record.use_time,record.is_use,record.coupon_type,coupon.title,coupon.is_weixin,user.wechaname,user.tel')
