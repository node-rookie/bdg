...
class ShakearoundAction extends UserAction
{
	...
	public function index()
	{
		...
		$this->update_status($Page->firstRow, $Page->listRows, $token);
	...
	public function apply_device()
	{
		if (IS_POST) {
			...
			if (($result['errcode'] == 0) && ($result['errmsg'] == 'success.')) {
				...
				$add_device = M('shakearoung_device')->add($device);
	...
	public function update_status($token = '')
	{
		...
		if ($status != 0) {
			...
			if (!empty($exists) && ($exists['status'] != 0)) {
				$db_device->where(array('device_id' => $first_result['data']['devices'][0]['device_id']))->save(array('status' => $status));
			}
		}
		...
		if (1 < $first_result['data']['total_count']) {
			...
			if (($update_result['errcode'] == 0) && ($update_result['errmsg'] == 'success.')) {
				...
				foreach ($update_result['data']['devices'] as $key => $val) {
					if ($val['status'] != 0) {
						$db_device->where(array('device_id' => $val['device_id']))->save(array('status' => $val['status']));
					}
				}
			}
		}
	...
	public function manual_update($token = '')
	{
		if (empty($token)) {
			return false;
		}
		...
		$db_device = M('shakearoung_device');
		$total = $db_device->where(array('token' => $token))->count();
