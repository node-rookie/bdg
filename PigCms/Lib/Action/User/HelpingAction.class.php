...
class HelpingAction extends UserAction{
	...
	public function rank(){
		...
		foreach ($list as $key => $val) {
			...
			//邀请人查询开始
			$shareuserinfo = M('Helping_record')->where(array('token'=>$this->token,'pid'=>$id,'wecha_id'=>$val['wecha_id']))->field('share_key')->find();
			$shareuser = M('Helping_user')->where(array('token'=>$this->token,'pid'=>$id,'share_key'=>$shareuserinfo['share_key']))->find();
			...
			$list[$key]['sharecount']  	= M('Share')->where(array('token'=>$this->token,'wecha_id'=>$val['wecha_id'],'moduleid'=>$id,'module'=>'Helping'))->count();
