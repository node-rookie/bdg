...
<?php if($choujiang["title"] != ''): ?><div class="contents">
	<div class="content-wrapper clearfix">
	  <a href="index.php?g=Wap&m=Card&a=scorelotteryview&token=<?php echo ($token); ?>&wecha_id=<?php echo ($wecha_id); ?>&cardid=<?php echo ($thisCard["id"]); ?>&aid=<?php echo ($choujiang["id"]); ?>">
	  <article class="article">
		<div class="article-content">
			<div class="article-logo-wrapper">
				<span class="article-logo bg-img" style="background-image:url(<?php echo ($choujiang["pic"]); ?>)"></span>
			</div>
			<div class="btn-container disabled">
				<span class="exchange-btn">立即抽奖</span>
			</div>
			<div class="article-info-container">
				<div class="info-inline-container">
					<p class="info-title">【积分抽奖】<?php echo ($choujiang["title"]); ?></p>
					<p class="info-inline">总量:<?php echo ($choujiang["people"]); ?>件</p>
					<p class="info-inline">剩余:<?php echo ($choujiang['people']-$choujiang['usetime']);?>件</p>
					<p class="info-explain">所需积分:<?php echo ($choujiang["integral"]); ?>积分</p>
				</div>
...
<div class="giftListBox" id="giftList">
<ul>
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><li>
	<a href="index.php?g=Wap&m=Card&a=signcpview&token=<?php echo ($token); ?>&wecha_id=<?php echo ($wecha_id); ?>&cardid=<?php echo ($thisCard["id"]); ?>&aid=<?php echo ($item["id"]); ?>">
    <div class="liBox">
            <div class="imgBox-wrap">
                <div class="imgBox">
                    <img class="imgBox-img" src="<?php echo ($item["pic"]); ?>" title="<?php echo ($item["title"]); ?>" />
                </div>
            </div>
            <div class="info">
                <div class="title"><?php echo ($item["title"]); ?></div>
                <div class="ext">
				<span class="gift-list-nums">剩<i class="nums"><?php echo ($item['people']-$item['usetime']);?></i>件<span class="gift-list-need"><i class="needs"><?php echo ($item["integral"]); ?></i>积分</span>
                </div>
            </div>
    </div>
	</a>
