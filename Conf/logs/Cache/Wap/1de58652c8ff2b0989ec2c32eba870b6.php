...
<body onload="init()">
	...
	//结束后调用 _gameScore 点中的数量
		...
		$.ajax({
			url: "<?php echo U('AppleGame/gameOver',array('token'=>$_GET['token'],'wecha_id'=>$_GET['wecha_id'],'id'=>intval($_GET['id'])));?>",
			type: "post",
			data: {"num":_gameScore},
		...
		l.style.display = 'block';
		window.shareData.tTitle = '哈哈，七夕佳节，走鹊桥找织女。我居然得了'+_gameScore+'分!不服来挑战！！！'
	...
	function replayBtn(){
		location.href="<?php echo U('AppleGame/index',array('token'=>$_GET['token'],'id'=>intval($_GET['id']),'wecha_id'=>$_GET['wecha_id']));?>";
		//gameRestart();
		//hideGameScoreLayer();
	}
	...
	function shareText( score ){
		...
		return '哈哈，七夕佳节，走鹊桥找织女。我居然得了'+score+'分！<br/>';
	}
...
<!--	游戏结束	-->
	<div style="display:none;background:url(./tpl/static/applegame/gameoverbg.jpg) no-repeat center 0 #031448;background-size:100%;" id="GameScoreLayer" class="BBOX SHADE">
		<div style="padding:40% 5%;">
			...
			<div id="GameScoreLayer-btn" class="BOX">
				<div style="width:142px;height:48px;" class="btn BOX-S" onclick="replayBtn()"><img src="./tpl/static/applegame/btn_again.png" width="100%" /></div>&nbsp;
				<div style="width:142px;height:48px;" class="btn BOX-S" onclick="share()"><img src="./tpl/static/applegame/btn_share.png" width="100%" /></div>&nbsp;
...
window.shareData = {  
            ...
            "timeLineLink": "<?php echo ($f_siteUrl); echo U('AppleGame/index',array('token'=>$_GET['token'],'id'=>intval($_GET['id'])));?>",
            "sendFriendLink": "<?php echo ($f_siteUrl); echo U('AppleGame/index',array('token'=>$_GET['token'],'id'=>intval($_GET['id'])));?>",
            "weiboLink": "<?php echo ($f_siteUrl); echo U('AppleGame/index',array('token'=>$_GET['token'],'id'=>intval($_GET['id'])));?>",
