...
<?php if(is_array($products)): $i = 0; $__LIST__ = $products;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$hostlist): $mod = ($i % 2 );++$i;?><li>
		<span class="pic">
		<a href="<?php echo U('Storenew/biddingproduct',array('token' => $_GET['token'], 'id' => $hostlist['id'], 'wecha_id' => $_GET['wecha_id'], 'twid' => $twid));?>">
			<img src="<?php echo ($hostlist["logourl"]); ?>" data-original="<?php echo ($hostlist["logourl"]); ?>">
		</a>
		</span>
		<span class="con">
			<a class="t" href="<?php echo U('Storenew/biddingproduct',array('token' => $_GET['token'], 'id' => $hostlist['id'], 'wecha_id' => $_GET['wecha_id'], 'twid' => $twid));?>"><?php echo ($hostlist["name"]); ?></a>
			<span style="color:#c00;font-size:12px;margin-top:0px;">当前出价：￥<?php echo ($hostlist["price"]); ?> 元</span>
			<span style="color:#999;font-size:12px;margin-top:3px;">参与人次：已 <?php echo ($hostlist['logcount']); ?> 人参与出价</span>
			<?php if($hostlist["endtime"] > $now): ?><span style="color:#999;font-size:12px;margin-top:3px;">开始日期：<?php echo (date("Y-m-d H:i:s",$hostlist["starttime"])); ?></span>
			<span style="color:#999;font-size:12px;margin-top:3px;">截止日期：<?php echo (date("Y-m-d H:i:s",$hostlist["endtime"])); ?></span>
			<?php else: ?>
			<span style="color:#c00;font-size:12px;margin-top:3px;">本次竞拍活动已结束</span><?php endif; ?>
		</span>
	</li><?php endforeach; endif; else: echo "" ;endif; ?>
</ul> -->

<section class="main-view" style="display: block;">
            <div id="tuan" class="tuan" style="padding-top: 10px; display: block;">
                <div style="margin-bottom: 10px;">
					<?php if(is_array($products)): $i = 0; $__LIST__ = $products;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$hostlist): $mod = ($i % 2 );++$i;?><div>
						<div class="tuan_g" style="border: 1px solid #ececec">
							<a href="<?php echo U('Storenew/grouponview',array('token' => $_GET['token'], 'id' => $hostlist['id'], 'wecha_id' => $_GET['wecha_id'], 'twid' => $twid));?>">
								<div class="tuan_g_img">
									<img src="<?php echo ($hostlist["topimg"]); ?>">
									<span class="tuan_mark tuan_mark2">
									<b>7.5折</b>
									<span><?php echo ($hostlist["tgnum"]); ?>人团</span>
									</span>
								</div>
								<div class="tuan_g_info">
									<p class="tuan_g_name"><?php echo ($hostlist["name"]); ?></p>
									<p class="tuan_g_cx">【限时特价】<?php echo ($hostlist["name"]); ?></p>
								</div>
								<div class="tuan_g_core">
									<div class="tuan_g_price">
										<span><?php echo ($hostlist["tgnum"]); ?>人团</span>
										<b>¥<?php echo ($hostlist["tgprice"]); ?></b>
									</div>
									<div class="tuan_g_btn">去开团</div>
								</div>
								<div class="tuan_g_mprice">
									单购价：¥<?php echo ($hostlist["price"]); ?>
								</div>
							</a>
						</div>
