        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><li data-card>
            <a href="javascript:;" onclick="this.classList.toggle('toggle');" <?php if($item["is_use"] == 1): ?>class="on"<?php endif; ?>>
              <header>
                ...
                <label class="fr">
                  <?php if($item["is_use"] == 0): ?>剩余<?php endif; echo ($item["count"]); ?>张 
                </label>
              </header>
              <section>
                <div>
                  ...
                  <div>
                    ...
                    <?php else: ?> 
                      <span onclick="location.href='<?php echo U('Card/coupon_use',array('token'=>$token,'wecha_id'=>$wecha_id,'cardid'=>$thisCard['id'],'coupon_id'=>$item['id'],'coupon_type'=>$item['coupon_type']));?>';event.stopPropagation();">
                        <label>
                          立即<br/>使用
                        </label>
                      </span><?php endif; ?>
