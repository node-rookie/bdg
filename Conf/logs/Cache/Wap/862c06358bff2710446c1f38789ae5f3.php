...
<title><?php echo ($thisCard["cardname"]); ?>积分兑换实物</title>
...
<header>
	<nav id="nav_1" class="p_10">
		<ul class="boxb">
			<li><a href="index.php?g=Wap&m=Card&a=signcpenter&token=<?php echo ($token); ?>&wecha_id=<?php echo ($wecha_id); ?>&cardid=<?php echo ($thisCard["id"]); ?>&type=<?php echo ($type); ?>&is_use=0" class="<?php if($is_use == '0'): ?>on<?php endif; ?>">未使用</a></li>
			<li><a href="index.php?g=Wap&m=Card&a=signcpenter&token=<?php echo ($token); ?>&wecha_id=<?php echo ($wecha_id); ?>&cardid=<?php echo ($thisCard["id"]); ?>&type=<?php echo ($type); ?>&is_use=1" class="<?php if($is_use == '1'): ?>on<?php endif; ?>">已使用</a></li>
		</ul>
	</nav>
</header>
...
function use(i){
	...
	jQ.post('/index.php?g=Wap&m=Card&a=action_usesigncp&wecha_id=<?php echo ($wecha_id); ?>&token=<?php echo ($token); ?>&cardid=<?php echo ($thisCard["id"]); ?>', submitData,
	function(data) {
		if (data.success == 1) {
