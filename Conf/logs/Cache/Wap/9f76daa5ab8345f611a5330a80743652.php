...
<div class="container coupon exchange <?php if($list == ''): ?>none<?php endif; ?>">
  <header>
  <nav id="nav_1" class="p_10">
  <ul class="box">
    <li><a href="<?php echo U('Card/integral',array('token'=>$token,'wecha_id'=>$wecha_id,'cardid'=>$thisCard['id'],'type'=>$type,'is_use'=>0));?>"  <?php if($is_use == 0): ?>class="on"<?php endif; ?>>未使用</a></li>
    <li><a href="<?php echo U('Card/integral',array('token'=>$token,'wecha_id'=>$wecha_id,'cardid'=>$thisCard['id'],'type'=>$type,'is_use'=>1));?>" <?php if($is_use == 1): ?>class="on"<?php endif; ?>>已使用</a></li>
  </ul>
  </nav>
  </header>
  <div class="body">
    <ul class="list_exchange">
      <ol>
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><li data-card onclick="this.classList.toggle('on');" >
          <header>
            <ul class="tbox">
              <li>
                <h5><?php echo ($item["title"]); ?>&nbsp;(<?php echo ($item["count"]); ?>张)</h5>
          ...
          <footer>
            <dl class="box">
              ...
              <dd>
                <?php if($item["count"] > 0): ?><a href="<?php echo U('Card/coupon_use',array('token'=>$token,'wecha_id'=>$wecha_id,'cardid'=>$thisCard['id'],'coupon_id'=>$item['id'],'coupon_type'=>$item['coupon_type']));?>" onclick="event.stopPropagation();">立即使用</a><?php endif; ?>
              </dd>
            </dl>
          </footer>
