...
<?php if(is_array($products)): $i = 0; $__LIST__ = $products;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$p): $mod = ($i % 2 );++$i; if(empty($p['detail']) != true): if(is_array($p['detail'])): $i = 0; $__LIST__ = $p['detail'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><li number="1">
				<span class="pic"><a href="<?php echo U('Storenew/product',array('token'=>$_GET['token'],'id'=>$p['id'],'wecha_id'=>$_GET['wecha_id'], 'twid' => $twid));?>"><img src="<?php echo ($p["logourl"]); ?>" width="75" height="75"></a></span>
				<span class="con">
					<a href="<?php echo U('Storenew/product',array('token'=>$_GET['token'],'id'=>$p['id'],'wecha_id'=>$_GET['wecha_id'], 'twid' => $twid));?>" class="t"><?php echo ($p["name"]); ?></a>
					...
					<!-- <label style="cursor:pointer" onclick="location.href='<?php echo U('Storenew/deleteCart',array('token'=>$_GET['token'],'id'=>$p['id'],'did'=>$row['id'],'wecha_id'=>$_GET['wecha_id']));?>'" class="del">删除</label> -->
					<label style="cursor:pointer" onclick="location.href='<?php echo U('Storenew/deleteCart',array('token'=>$_GET['token'],'id'=>$p['id'],'did'=>$row['id'],'wecha_id'=>$_GET['wecha_id'], 'twid' => $twid));?>'" class="del">删除</label>
					</p>
				</span>
	...
	<?php else: ?>
		<li number="<?php echo ($p["count"]); ?>">
			...
			<span class="con">
				...
				<label style="cursor:pointer" onclick="location.href='<?php echo U('Storenew/deleteCart',array('token'=>$_GET['token'],'id'=>$p['id'],'did'=>0,'wecha_id'=>$_GET['wecha_id'], 'twid' => $twid));?>'" class="del">删除</label>
				</p>
			</span>
...
<p class="act">
	<a href="<?php echo U('Storenew/index',array('token'=>$_GET['token'], 'wecha_id'=>$_GET['wecha_id'], 'twid' => $twid,'cid' => $cid));?>" class="back">继续购物<i></i></a>
	<a href="<?php echo U('Storenew/orderCart',array('token'=>$_GET['token'], 'wecha_id'=>$_GET['wecha_id'], 'twid' => $twid,'cid' => $cid));?>" class="checkout">下单结算</a>
</p>
</div>
<?php else: ?>
<div class="m-cart-e">
...
<a href="<?php echo U('Storenew/cats',array('token'=>$_GET['token'],'wecha_id'=>$_GET['wecha_id'], 'twid' => $twid, 'cid' => $cid));?>" class="gobuy">去挑选商品</a>
</div><?php endif; ?>
...
function update_cart(rowid, num, did) {
	...
	$.ajax({
		url: '<?php echo U('Storenew/ajaxUpdateCart',array('token'=>$_GET['token'],'wecha_id'=>$_GET['wecha_id'], 'twid' => $twid));?>&id='+rowid+'&count='+num+'&did='+ did,
...
window.shareData = {  
            ...
            "moduleID":"0",
            "imgUrl": "", 
            "timeLineLink": "<?php echo ($f_siteUrl); echo U('Storenew/cart',array('token' => $_GET['token'], 'twid' => $mytwid, 'cid' => $cid));?>",
            "sendFriendLink": "<?php echo ($f_siteUrl); echo U('Storenew/cart',array('token' => $_GET['token'], 'twid' => $mytwid, 'cid' => $cid));?>",
            "weiboLink": "<?php echo ($f_siteUrl); echo U('Storenew/cart',array('token' => $_GET['token'], 'twid' => $mytwid, 'cid' => $cid));?>",
