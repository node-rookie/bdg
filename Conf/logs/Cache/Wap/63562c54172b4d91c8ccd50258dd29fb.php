...
function  updata(){
	...
	if(last_count<count){
		$.ajax({
			type: "post",
			url : "<?php echo U('Shake/refreshScreen',array('act_type'=>$act_type,'token'=>$token,'id'=>$info['id'],wecha_id=>$wecha_id));?>",
			dataType:'text',
			data: 'count='+count,
...
function checkStart(){
    ...
    $.post('index.php?g=Wap&m=Shake&a=shakeActivityStatus&id=<?php echo ($info["id"]); ?>&token=<?php echo ($info["token"]); ?>&wecha_id=<?php echo ($wecha_id); ?>', submitData,
    function(data) {
        if (data.isact == 1) {
