...
<div class="wrapper">
	...
	<div class="question" style="overflow:hidden;">
		...
		<div class="options">
			<?php if(is_array($answer)): $i = 0; $__LIST__ = $answer;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><div class="option" data-value="<?php echo ($row['id']); ?>">
...
$(function(){
	...
	$("#submit").click(function(){
		...
		$.ajax({
			type : "POST",
			url : "<?php echo U('Research/answer', array('reid' => $rid, 'token' => $token, 'wecha_id' => $wecha_id));?>",
			data : submitData,
			...
			success : function(data){
				if(data.error_code == true){
					...
					window.location.href = "<?php echo U('Research/answer', array('reid' => $rid, 'token' => $token, 'wecha_id' => $wecha_id));?>";
				}
			},
			error : function(data){
