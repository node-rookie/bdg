...
<?php if($ordersCount != 0): if(is_array($orders)): $i = 0; $__LIST__ = $orders;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$o): $mod = ($i % 2 );++$i;?><ul class="m-uc-order-li">
			<li>提现的时间：<span style="color:green"><?php echo (date("Y-m-d H:i:s",$o['dateline'])); ?></span></li>
			<li>提现的金额：<span style="color:green"><?php echo ($o['price']); ?></span>　元</li>
			<li>提现账号：<span style="color:green"><?php echo ($o['number']); ?></span></li>
			<li>提现银行：<span style="color:green"><?php echo ($o['bank']); ?></span></li>
			<li>提现状态：<?php if($o['status'] == 1): ?><span style="color:green">提现成功</span><?php else: ?><span style="color:red">审核中</span><?php endif; ?></li>
		</ul><?php endforeach; endif; else: echo "" ;endif; ?>
	<?php if($totalpage > 1) { ?>
		<div class="m-page uc-orderlist">
			<?php if($page == 1): ?><div class="pg-pre pg-grey"><a href="javascript:void(0);">上一页<i><em></em></i></a></div>
			<?php else: ?>
				<div class="pg-pre"><a href="<?php echo U('Store/remove', array('token'=>$token,'page'=>intval($page - 1),'wecha_id'=>$wecha_id, 'twid' => $twid, 'cid' => $cid));?>">上一页<i><em></em></i></a></div><?php endif; ?>
			...
			<?php else: ?>
				<div class="pg-next"><a href="<?php echo U('Store/remove', array('token'=>$token,'page'=>intval($page + 1),'wecha_id'=>$wecha_id, 'twid' => $twid, 'cid' => $cid));?>">下一页<i><em></em></i></a></div><?php endif; ?>
		</div>
	<?php }else{} ?>
<?php else: ?>
	<ul style="margin: 15px 10px;border-radius: 4px;line-height: 1.4em;font-size: 16px;border: 1px solid #ddd;background: #f5f5f5;padding: 10px 10px 6px;">
		<span><li class="p-li" style="padding:10px; text-align:center;">没有提现记录</li></span>
	</ul><?php endif; ?>
</body>
...
window.shareData = {  
            ...
            "moduleID":"",
            "imgUrl": "", 
            "timeLineLink": "<?php echo ($f_siteUrl); echo U('Store/remove',array('token' => $_GET['token'], 'twid' => $mytwid, 'cid' => $cid));?>",
            "sendFriendLink": "<?php echo ($f_siteUrl); echo U('Store/remove',array('token' => $_GET['token'], 'twid' => $mytwid, 'cid' => $cid));?>",
            "weiboLink": "<?php echo ($f_siteUrl); echo U('Store/remove',array('token' => $_GET['token'], 'twid' => $mytwid, 'cid' => $cid));?>",
