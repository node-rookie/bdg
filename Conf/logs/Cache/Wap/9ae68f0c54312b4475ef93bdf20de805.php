...
<li class="nob">
	<div class="beizhu">请先完善您的真实个人信息，红色为必填项</div>
</li>
...
<td><input name="nickname" type="text" class="px" id="nickname" value="<?php echo ($info["nickname"]); ?>" placeholder="请输入您的微信名称"></td>
</tr>
</table>
</li>
...
<td><input name="phone"  class="px" id="phone" value="<?php echo ($info["phone"]); ?>"  type="text" placeholder="请输入您的电话"></td>
</tr>
</table>
</li>
...
<div class="por">
	<img src="<?php if($info["portrait"] != ''): echo ($info["portrait"]); else: ?>/tpl/User/default/common/images/portrait.jpg<?php endif; ?>" id="portrait_src" />
</div>
...
function() {
    ...
    $.getJSON('index.php?g=Wap&m=Scene_member&a=set&token=<?php echo ($token); ?>&wecha_id=<?php echo ($wecha_id); ?>&act_type=<?php echo ($act_type); ?>&act_id=<?php echo ($act_id); ?>&name=<?php echo ($name); ?>', submitData,
    function(res) {
        if(res.err == 0 && res.name == 'shake'){
        	showTip('报名成功，2秒后自动跳转摇一摇',res.href);
        	
        	/*
        	zAlert.Alert({
				closed: false,
				content: '报名成功，点击确定参加摇一摇',
				callback: function() {
					zAlert.Close();
    		...
    		*/
		}else if(res.err == 0 && res.name == 'wall'){
			showTip('报名成功，请关闭窗口返回微信输入框','');
			/*
			zAlert.Alert({
				closed: false,
				content: '报名成功，请关闭窗口进入微信输入框',
				callback: function() {
					zAlert.Close();
					CloseWebPage();
				}
			...
			*/
		}else if(res.err == 0 && res.name == 'vote'){
			showTip('报名成功，2秒后自动跳转投票',res.href);
			/*
        	zAlert.Alert({
				closed: false,
				content: '报名成功，点击确定参加投票',
				callback: function() {
					zAlert.Close();
					window.location.href=res.href;
				}
			...
			showTip('报名成功，请关闭窗口后进入活动','');
			/*
			zAlert.Alert({
				closed: false,
				content: '报名成功，请关闭窗口后进入活动',
				callback: function() {
					zAlert.Close();
					CloseWebPage();
				}
