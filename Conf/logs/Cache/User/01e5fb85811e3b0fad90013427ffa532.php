...
<div class="content">
	<div class="cLineB">
		<h4 class="left">商品【<?php echo ($unitary["name"]); ?>】的数据</h4>
	...
	<div class="msgWrap">
		<table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
			...
			<tbody>
				<?php if(is_array($lucknum_list)): $i = 0; $__LIST__ = $lucknum_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
					...
					<?php
 $thistime = floor($vo['addtime']/1000); $ms = substr($vo['addtime'],-3); ?>
					<td><?php echo date("Y-m-d H:i:s",$thistime).".".$ms;?></td>
					<td class="norightborder">
						<?php if($vo['state'] == 1 && $vo['paifa'] == 0){?>
						<a href="<?php echo U('Unitary/paifa',array('token'=>$token,'lucknumid'=>$vo['id']));?>">发货</a>
						<?php }elseif($vo['state'] == 1 && $vo['paifa'] == 1){?>
						<a href="<?php echo U('Unitary/paifa',array('token'=>$token,'lucknumid'=>$vo['id']));?>">已发货</a>
						<?php }else{echo "无";}?>
					</td>
