...
<select onchange="change(this);">
	<?php if(is_array($dates)): $i = 0; $__LIST__ = $dates;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$d): $mod = ($i % 2 );++$i; if($d['k'] == $reservetime): ?><option value="<?php echo ($d['k']); ?>" selected="selected"><?php echo ($d['v']); ?></option>
	<?php else: ?>
...
<td><?php echo ($o["tname"]); ?></td>
<td><?php if(isset($o['reservetimestr'])): echo ($o['reservetimestr']); else: echo (date("Y-m-d H:i:s",$o["reservetime"])); endif; ?></td> 
<td><span class="red"><?php echo ($o["name"]); ?></span>  <?php if($o['sex'] == 1): ?>先生<?php else: ?>女士<?php endif; ?> 预定。总金额：<?php echo ($o["price"]); ?>（<?php if($o['paid'] == 1): ?>已支付<?php else: ?>未支付<?php endif; ?>）</td> 
<td><?php echo ($o["tel"]); ?></td> 
<td><?php if($o['isuse'] == 1): ?><span style="color:green">正在就餐...</span><?php elseif($o['isuse'] == 2): ?><span style="color:red">已就餐过了</span><?php else: ?><span style="color:red">未就餐</span><?php endif; ?> <?php if($o['isuse'] != 2): ?><a class="btnGray vm" style="margin-left:20px;" onclick="toSwitchStatus(2,<?php echo ($o['tableid']); ?>,<?php echo ($o['isuse']); ?>,<?php echo ($o['id']); ?>)">更新就餐状态</a><?php endif; ?></td>
