...
<ul>
	...
	<li class="current tabli" id="tab2"><a href="<?php echo U('Storenew/detailinfo',array('token'=>$token));?>">查看分佣信息</a></li>
...
<td><?php echo ($hostlist['ordername']); ?></td>
<td>
	<?php if($hostlist['type'] == 1): ?>点击
	<?php elseif($hostlist['type'] == 2): ?> 注册
	<?php else: ?> 购买<?php endif; ?>
</td> 
<td><?php echo (date("Y-m-d H:i:s",$hostlist['dateline'])); ?></td> 
<td><?php echo ($hostlist['totalprice']); ?> 元</td>
<td><?php echo ($hostlist['price']); ?> 元</td>
<td><?php if($hostlist['type'] == 3): ?>一级推广<?php elseif($hostlist['type'] == 4): ?>二级推广<?php endif; ?></td>
