...
afterBlur: function(){this.sync();}
  ...
  <div class="content"> 
   <div class="cLineB"> 
    <h4>商品设置</h4> 
    <a href="<?php echo U('Storenew/product',array('token'=>$token,'catid'=>$catid));?>" class="right  btnGreen" style="margin-top:-27px">返回</a> 
   </div> 
     ...
     <table class="userinfoArea" style=" margin:0;" border="0" cellspacing="0" cellpadding="0" width="100%"> 
      <tbody> 
       ...
       <tr> 
        <th>类别：</th> 
        <td>
		<select name="catid" id="catid" style="width:170px;padding: 0 2px">
		<?php if(is_array($cat)): $i = 0; $__LIST__ = $cat;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cat): $mod = ($i % 2 );++$i;?><option value="<?php echo ($cat["id"]); ?>" <?php if($set['catid'] == $cat['id']): ?>selected<?php endif; ?>><?php echo ($cat["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
		</select>
		</td> 
       </tr>
       ...
       <tr>
           <th>单独设置分销：</th>
           <td><input type="checkbox" name="allow_distribution" class="allow_distribution" value="1" <?php if($set['allow_distribution'] == 1): ?>checked="true"<?php endif; ?> /></td>
       </tr>
       <tr class="distribution-tr" <?php if($set['allow_distribution'] != 1): ?>style="display: none"<?php endif; ?>>
           <th>1级分销佣金：</th>
           <td>
		   <select name="commission_type" id='commission_type' style="width:300px;padding: 0 2px">
				<option value="default" <?php if($set['commission_type'] == 'default'): ?>selected<?php endif; ?>>默认后台设置，不单独设置佣金，此处不生效</option>
				<option value="float" <?php if($set['commission_type'] == 'float'): ?>selected<?php endif; ?>>百分比 % (佣金 = 商品分销价格 * 佣金比率)</option>
				<option value="fixed" <?php if($set['commission_type'] == 'fixed'): ?>selected<?php endif; ?>>固定金额 元</option>
			</select>
			<br>
			<input type="text" style="width: 50px" class="px commission_fixed" name="commission" id="commission" value="<?php if($set["commission"] > 0): echo ($set["commission"]); endif; ?>"/></span>
            </td>
       </tr>
	   <tr class="distribution-tr" <?php if($set['allow_distribution'] != 1): ?>style="display: none"<?php endif; ?>>
		   <th>2级分销佣金：</th>
           <td>
		   <select name="addcommission_type" id='addcommission_type' style="width:300px;padding: 0 2px">
				<option value="default" <?php if($set['addcommission_type'] == 'default'): ?>selected<?php endif; ?>>默认后台设置，不单独设置佣金，此处不生效</option>
				<option value="float" <?php if($set['addcommission_type'] == 'float'): ?>selected<?php endif; ?>>百分比 % (佣金 = 商品分销价格 * 佣金比率)</option>
				<option value="fixed" <?php if($set['addcommission_type'] == 'fixed'): ?>selected<?php endif; ?>>固定金额 元</option>
			</select>
			<br>
			<input type="text" style="width: 50px" class="px commission_fixed" name="addcommission" id="addcommission" value="<?php if($set["addcommission"] > 0): echo ($set["addcommission"]); endif; ?>"/></span>
            </td>
       </tr>
	   
	   <tr>
           <th>参与团购：</th>
           <td><input type="checkbox" name="is_tg" id="is_tg" class="tg" value="1" <?php if($set['is_tg'] == 1): ?>checked="true"<?php endif; ?> /></td>
       </tr>
       <tr class="tg-tr" <?php if($set['is_tg'] != 1): ?>style="display: none"<?php endif; ?>>
           <th>团购价格：</th>
           <td>
			<input type="text" style="width: 50px" class="px" id="tgprice" name="tgprice" value="<?php if($set["tgprice"] > 0): echo ($set["tgprice"]); endif; ?>"/>
            </td>
       </tr>
	   <tr class="tg-tr" <?php if($set['is_tg'] != 1): ?>style="display: none"<?php endif; ?>>
		   <th>成团人数：</th>
           <td>
			<input type="text" style="width: 50px" class="px" id="tgnum" name="tgnum" value="<?php if($set["tgnum"] > 0): echo ($set["tgnum"]); endif; ?>"/>
            </td>
       </tr>
	   <tr class="tg-tr" <?php if($set['is_tg'] != 1): ?>style="display: none"<?php endif; ?>>
		   <th>团购关闭时间：</th>
           <td>
			<input type="text" style="width: 50px" class="px" id="tgend" name="tgend" value="<?php if($set["tgend"] > 0): echo ($set["tgend"]); endif; ?>"/> 天(多少天后删除未付款的团购订单并退款到会员卡余额)
            </td>
       </tr>
	   ...
	   <tr> 
        <th>是否新品：</th> 
        <td>
	        <input type="radio" name="xinpin" class="xinpin" value="0" id="xinpin_0" <?php if($set['xinpin'] == 0): ?>checked<?php endif; ?> /><label for="xinpin_0">否</label>&nbsp;&nbsp;&nbsp;&nbsp;
	        <input type="radio" name="xinpin" class="xinpin" value="1" id="xinpin_1" <?php if($set['xinpin'] == 1): ?>checked<?php endif; ?> /><label for="xinpin_1">是</label>
		</td> 
       </tr>
	   <tr> 
        <th>是否限时：</th> 
        <td>
	        <input type="radio" name="xianshi" class="xianshi" value="0" id="xianshi_0" <?php if($set['xianshi'] == 0): ?>checked<?php endif; ?> /><label for="xianshi_0">否</label>&nbsp;&nbsp;&nbsp;&nbsp;
	        <input type="radio" name="xianshi" class="xianshi" value="1" id="xianshi_1" <?php if($set['xianshi'] == 1): ?>checked<?php endif; ?> /><label for="xianshi_1">是</label>
		</td> 
       </tr>
	   <tr> 
        <th>是否推荐：</th> 
        <td>
	        <input type="radio" name="tuijian" class="tuijian" value="0" id="tuijian_0" <?php if($set['tuijian'] == 0): ?>checked<?php endif; ?> /><label for="tuijian_0">否</label>&nbsp;&nbsp;&nbsp;&nbsp;
	        <input type="radio" name="tuijian" class="tuijian" value="1" id="tuijian_1" <?php if($set['tuijian'] == 1): ?>checked<?php endif; ?> /><label for="tuijian_1">是</label>
		</td> 
       </tr>
       <tr> 
        <th>邮费：</th> 
       ...
       <th>&nbsp;</th>
       <td>
       <button type="button" name="button" class="btnGreen" id="save">保存</button> &nbsp; <a href="<?php echo U('Storenew/index',array('token'=>$token));?>" class="btnGray vm">取消</a></td> 
       </tr> 
      </tbody> 
     </table> 
     </div>
...
$(document).ready(function(){
	...
	$("#save").click(function(){
		...
		var tgnum = $("#tgnum").val();
		var tgend = $("#tgend").val();
		var addcommission_type = $("#addcommission_type").val();
		var addcommission = $("#addcommission").val();
		var price = $("#price").val();
		...
		var xianshi = $(".xianshi:checked").val();
		var xinpin = $(".xinpin:checked").val();
		var tuijian = $(".tuijian:checked").val();
		var attribute = [];
		var i = 0;
		var str = '';
		...
		var data = {pid:$("#pid").val(),
					...
					catid:catid,
					commission_type : addcommission_type,
                    commission : commission,
					addcommission_type : addcommission_type,
		...
		$.post('index.php?g=User&m=Storenew&a=productSave', data, function(response){
			if (response.error_code == false) {
				art.dialog({
				    ...
				    ok: function () {
				    	this.time(3);
				    	location.href='index.php?g=User&m=Storenew&a=product&token=<?php echo ($token); ?>&catid=<?php echo ($catid); ?>'
				        return false;
				    },
				    cancelVal: '关闭'
				});
