...
<div class="msgWrap bgfc">
  <div class="affirm">
    <?php if($status == 4): ?><p class="affirm_err">项目已结束，您无法修改信息！</p>
    <?php elseif($status == 3): ?>
      <p class="affirm_err">项目已发布，请不要重复发布！</p>
      <p>点击“上一步”可以修改信息</p>
    <?php elseif($status == 2): ?>
      <p class="affirm_tit">您的项目已经设置  完成！</p>
      <p>“上一步”可以返回修改信息</p>
      <p>“确认发布”后将计时开启项目</p>
    <?php else: ?>
      <p class="affirm_err">请设置完整信息再发布</p><?php endif; ?>
  </div>

  <div class="common_button">
    <input type="hidden" name="pid" id="pid" value="<?php echo ($pid); ?>">
    <button type="button"  class="button" onclick="javascript:window.location='<?php echo U('Crowdfunding/reward_set',array('token'=>$token,'pid'=>$pid));?>';">上一步</button>
    <button type="submit" class="btnGreen">确认发布</button>
  </div>
</div>
</form>
