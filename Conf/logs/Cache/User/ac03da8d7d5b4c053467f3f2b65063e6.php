...
<div class="cLineB">
    <h4>回报设置</h4>
    <a href="<?php echo U('Crowdfunding/index',array('token'=>$token));?>"  class="right btnGrayS vm" style="margin-top:-27px" >返回</a>
</div>
...
<table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
  ...
  <tbody>
    <?php if(is_array($reward_list)): $i = 0; $__LIST__ = $reward_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
        <td><?php echo ($i); ?></td>
        <td><?php echo ($list["money"]); ?></td> 
        <td><?php echo ($list["people"]); ?></td>
        <td><?php echo ($list["content"]); ?></td>
        <td>项目结束后<?php echo ($list["back_day"]); ?>天</td>
        <td><?php echo ($list["carriage"]); ?></td>
        <td class="norightborder">
          <a href="<?php echo U('Crowdfunding/reward_set',array('token'=>$token,'pid'=>$pid,'id'=>$list['id']));?>">修改</a>
          <a href="<?php echo U('Crowdfunding/reward_del',array('token'=>$token,'pid'=>$pid,'id'=>$list['id']));?>" onclick="javascript:return confirm('确定要删除吗？');">删除</a>
        </td>
      </tr><?php endforeach; endif; else: echo "" ;endif; ?>
  </tbody>
</table>
<div class="w_adda">
  <?php if($set != ''): ?><h3 class="h3">添加回报选项</h3>
  <?php else: ?>
    <a href="javascript:void(0);" class="button addReward">添加回报</a><?php endif; ?>
</div>

<TABLE class="userinfoArea" style="<?php if($set != ''): ?>display:block;<?php else: ?>display:none;<?php endif; ?>" border="0" cellSpacing="0" cellPadding="0" width="100%">
<TBODY>
<TR>
  <th valign="top"><span class="red">*</span>支持金额：</th>
  <TD>
    <input type="input" class="px" id="warn" value="<?php echo ($set["money"]); ?>" name="money" style="width:120px" /> 元
  </TD>
</TR>

<TR>
  <th valign="top"><span class="red">*</span>回报内容：</th>
  <td><textarea  class="px" id="info" name="content"  style="width:320px; height:80px" ><?php echo ($set["content"]); ?></textarea>
  </td>
</TR>
...
<TR>
  ...
  <TD>
    <input class="px"  name="img" value="<?php if($set['img'] == ''): echo ($staticPath); ?>/tpl/static/crowdfunding/images/back.jpg<?php else: echo ($set["img"]); endif; ?>" id="pic1" style="width:280px;"  />
    <script src="/tpl/static/upyun.js"></script><a href="###" onclick="upyunPicUpload('pic1',400,400,'<?php echo ($token); ?>')" class="a_upload">上传</a>[400*400]<!--  <a href="###" onclick="viewImg('pic1')">预览</a> --><br />
    <img id="pic1_src" src="<?php if($set['img'] == ''): echo ($staticPath); ?>/tpl/static/crowdfunding/images/back.jpg<?php else: echo ($set["img"]); endif; ?>" height="120px" >
  </TD>
</TR>
...
<TR>
  ...
  <TD>
    <input type="input" class="px" id="warn" value="<?php echo ($set["people"]); ?>" name="people" style="width:120px" /> 人
  </TD>
</TR>
...
<TR>
  ...
  <td colspan="2">
    项目结束后<input type="input" class="px" id="warn" value="<?php if($set["back_day"] == ''): ?>10<?php else: echo ($set["back_day"]); endif; ?>" name="back_day" style="width:60px" /> 天发放
  </td>
</TR>
...
<TR>
  ...
  <td colspan="2">
    <input type="input" class="px" id="warn" value="<?php if($set["carriage"] == ''): ?>0<?php else: echo ($set["carriage"]); endif; ?>" name="carriage" style="width:120px" /> (“0”为包邮)
  </td>
</TR>
...
<TR>
  ...
  <td>
    ...
    <input name="id" type="hidden" id="id"  value="<?php echo ($set["id"]); ?>">
  ...
  <div class="common_button">
    <button type="button"  class="button" onclick="javascript:window.location='<?php echo U('Crowdfunding/basis_set',array('token'=>$token,'id'=>$pid));?>';">上一步</button> 
    <button type="button" class="button ml10"  onclick="javascript:window.location='<?php echo U('Crowdfunding/affirm',array('token'=>$token,'pid'=>$pid));?>';">下一步</button>
  </div>
</div>
