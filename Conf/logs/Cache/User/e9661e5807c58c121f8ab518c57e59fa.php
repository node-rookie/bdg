...
<style>
	...
	.album_list li .album_pic {
	display: block;
	width: 250px;
	height: 365px;
	overflow: hidden;
	}
	.album_list li .album_pic img {
	width: 250px;
	}
	.album_list li .album_main {
	padding: 10px;
	overflow: hidden;
	}
	.album_list li .album_main .album_title {
	font-size: 16px;
...
<h4 class="left">我的高级场景 </h4>

	</div>

	<a  target="_blank"href="<?php echo ($eqx_siteUrl); ?>/index.php?c=user&a=wfclogin&user=<?php echo ($token); ?>&password=<?php echo ($users["eqxpassword"]); ?>" class="right btnGrayS vm" style="margin-top:-47px">登入场景</a>
    <div class="info" style="height:150px;border: 1px solid rgb(239, 237, 237);"><!-- 
            <h3>我的场景</h3> -->
            <ul style="padding: 35px 20px;
background-color: #FFF;" ng-show="showScene" ng-init="showScene=true;showCustomer=false" class="clearfix">
                <li style="float:right;">
                    <a target="_blank" href="<?php echo ($eqx_siteUrl); ?>/index.php?c=user&a=wfccreat&user=<?php echo ($token); ?>&password=<?php echo ($users["eqxpassword"]); ?>" alt="创建场景" title="创建场景"><div class="chuangjian"><img ng-src="<?php echo ($eqx_siteUrl); ?>/Public/css/images/chuangjian.png" src="<?php echo ($eqx_siteUrl); ?>/Public/css/images/chuangjian.png"></div></a>
                </li>                 
                <li style="display: inline-block;
float: left;
margin-right: 30px;">
                    <div class="phone"  style="background: url(tpl/static/eqx/phone.png) 0 center no-repeat;
height: 80px;
padding-left: 100px;">
                        <h1 class="baseColor ng-binding" style="color: #08a1ef;font-size: 30px;
"><?php echo ($num); ?></h1>
                        <h2 style="font-size: 14px;
color: #9a9a9a;">场景总计</h2>
                    </div>
                </li>
                <li style="display: inline-block;
float: left;
margin-right: 30px;"><a href="#/main/spread">
                    <div class="eye" style="background: url(tpl/static/eqx/eye.png) 0 center no-repeat;
height: 80px;
padding-left: 100px;">
                        <h1 class="secondaryColor"><span class="ng-binding" style="color: #68dcc7;font-size: 30px;
padding: 10px 0;"><?php echo ($sum); ?></span></h1>
                        <h2 style="font-size: 14px;
color: #9a9a9a;">场景展示</h2>
                    </div></a>
                </li>
                <li style="display: inline-block;
float: left;
margin-right: 30px;">
                    <div class="show" style=" background: url(tpl/static/eqx/show.png) 0 center no-repeat;
height: 80px;
padding-left: 100px;">
                        <h1 class="secondaryColor"><span class="ng-binding" style="color: #9ad64b;font-size: 30px;
padding: 10px 0;"><?php echo ($date_count); ?></span></h1>
                        <h2 style="font-size: 14px;
color: #9a9a9a;">收集数据</h2>
                    </div>
                </li>               
            </ul>
        </div>
	<div>
		<ul class="unstyled album_list">
			<?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><li>
					<a target="_blank" href="<?php echo ($eqx_siteUrl); ?>/v-<?php echo ($item["scenecode_varchar"]); ?>" class="album_pic" style="height:250px">

<!--原声php判断输出-->
<?php
$d=substr($item['thumbnail_varchar'],0,1); $a=$item['thumbnail_varchar']; if($d==1){ echo "<img   src='$eqx_siteUrl/userfiles/$a', style='height:250px' >"; }else{ echo "<img src='$eqx_siteUrl/Uploads/$a', style='height:250px'>"; } ?>
<!--end-->
<?php
$d=substr($item['thumbnail_varchar'],0,1); $a=$item['thumbnail_varchar']; if($d==1){ $pic=''.$eqx_siteUrl.'/userfiles/'.$a.''; }else{ $pic=''.$eqx_siteUrl.'/Uploads/'.$a.''; } ?>

					</a>
                    <div style="height:50px; background-color:#08a1ef; line-height:50px; text-align:center; color:white;text-indent: 15px;font-size: 22px;"> <?php echo ($item["scenename_varchar"]); ?></div>
					<div class="album_main">
						<p class="album_title">场景展示：<a href="<?php echo ($eqx_siteUrl); ?>/index.php#/main/spread/statistics/<?php echo ($item['sceneid_bigint']); ?>" target="_blank" title="场景展示次数"><span style="color:#999"><?php echo ($item["hitcount_int"]); ?>次</span></a></p>
						<p class="album_title">收集数据：<a href="<?php echo ($eqx_siteUrl); ?>/index.php#/my/scene/<?php echo ($item['sceneid_bigint']); ?>#collectData" target="_blank" title="数据收集条数"><span style="color:#999"><?php echo ($item["datacount_int"]); ?>条</span></a></p>
                       
                        <p class="album_title">预览地址:<input style="border:none" class="px" value="<?php echo ($eqx_siteUrl); ?>/v-<?php echo ($item["scenecode_varchar"]); ?>" type="text"></p>
                         <p class="album_title"><a style="margin-left:0px" href="<?php echo U('Eqx/set',array('url'=>''.$eqx_siteUrl.'/v-'.$item['scenecode_varchar'].'','token'=>$token,'name'=>$item['scenename_varchar'],'desc_varchar'=>$item['desc_varchar'],'id'=>$item['sceneid_bigint'],'pic'=>$pic,'sceneid_bigint'=>$item['sceneid_bigint']));?>" title="回复配置" class="btnGrayS vm bigbtn">回复</a>
                         <a style="margin-left:20px" href="<?php echo ($eqx_siteUrl); ?>/index.php#/scene/create/<?php echo ($item['sceneid_bigint']); ?>?pageId=1"  title="回复配置" class="btnGrayS vm bigbtn" target="_blank">编辑</a>
                         <a style="margin-left:20px" href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Eqx/del',array('id'=>$item['sceneid_bigint'],'token'=>$token,'pid'=>$pid['id']));?>')" title="删除场景" class="btnGrayS vm bigbtn">删除</a></p>
