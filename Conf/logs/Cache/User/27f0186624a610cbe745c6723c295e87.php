...
<?php if($vote_list): if(is_array($vote_list)): $i = 0; $__LIST__ = $vote_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$m): $mod = ($i % 2 );++$i;?><tr><td><?php echo ($m["title"]); ?></td><td class="norightborder"><a href="###" onclick="returnHomepage(<?php echo ($m["id"]); ?>,'<?php echo ($m["title"]); ?>','<?php echo ($m["picurl"]); ?>')">选中</a></td></tr><?php endforeach; endif; else: echo "" ;endif; ?>
<?php else: ?>
<tr><td colspan="2" align="center"><a href="<?php echo U('Vote/add',array('type'=>'scene'));?>" target="_blank" style="color:#369">还没有现场投票，请点击这里添加投票</a></td></tr><?php endif; ?>
</table>
...
var imgids=art.dialog.data('vote_id');
// 返回数据到主页面
function returnHomepage(id,title,pic){
	var origin = artDialog.open.origin;
	var dom = origin.document.getElementById(titledom);
	var imgidsdom = origin.document.getElementById(imgids);
	dom.innerHTML=dom.innerHTML+title+'&nbsp;|&nbsp';
	dom.style.display="";
	imgidsdom.value+=','+id;
