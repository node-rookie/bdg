...
<tr>
  ...
  <td><textarea style="width:560px;height:75px" name="reply_content" id="info" class="px"><?php echo ($set["reply_content"]); ?></textarea><br/>最多填写128个字母(一个汉字相当于3个字母)</td>
</tr>
...
<tr>
	...
	<td><input type="input" class="px" id="start_time" value="<?php echo (date('Y-m-d',($set["start_time"])?($set["start_time"]):time())); ?>" onClick="WdatePicker()" name="start_time" />
		到
		<input type="input" class="px" id="end_time" value="<?php echo (date('Y-m-d',($set["end_time"])?($set["end_time"]): strtotime('+1 month'))); ?>" name="end_time" onClick="WdatePicker()"  />
	</td>
</tr>
</tbody>
</table>
  		...
  		<tbody>			
			<tr>
  				...
  				<td>
	  				...
	  				<input type="radio" name="need_follow" value="2" <?php if($set["need_follow"] == 2): ?>checked<?php endif; ?>>是
					<div style="float:right;margin-right:482px;"><?php echo ($__access_remind__); ?></div>
  				</td>
  			</tr>
			 <tr>
  				<th>是否需要粉丝信息：</th>
