...
<a href="<?php echo U('Voteimg/index',array('token'=>$token));?>" class="right btnGrayS vm" style="margin-top:1px">返回投票列表</a>
  ...
  <h2> 
	...
	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tbody><tr>
		  ...
		  <td><img src="<?php echo ($vo["vote_img"]); ?>" width="100px" height="100px"><input type="hidden" value="<?php echo ($vo["vote_img"]); ?>" id="img_<?php echo ($vo["id"]); ?>"/>&nbsp;&nbsp;&nbsp;&nbsp;<a onclick="viewImg('img_<?php echo ($vo["id"]); ?>')" href="###">预览</a></td>
		  <td><?php echo ($vo["contact"]); ?></td>
		  <td><?php echo (date('Y-m-d H:i',$vo["upload_time"])); ?></td>
		  <td><?php if($vo["check_pass"] == 0): ?>未审核<?php else: ?>审核通过<?php endif; ?></td>
		  <td class="norightborder">
		  <?php if($vo["check_pass"] == 0): ?><a href="<?php echo U('Voteimg/apply_check',array('vote_id'=>$vo['vote_id'],'token'=>$vo['token'],'item_id'=>$vo['id']));?>">通过</a><?php else: ?><a href="<?php echo U('Voteimg/apply_check',array('vote_id'=>$vo['vote_id'],'token'=>$vo['token'],'item_id'=>$vo['id']));?>">不通过</a><?php endif; ?><a href="<?php echo U('Voteimg/apply_del',array('vote_id'=>$vo['vote_id'],'token'=>$vo['token'],'item_id'=>$vo['id']));?>">删除</a>
		  </td>
		</tr>
