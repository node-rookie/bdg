...
<div class="content" <?php if(session('isQcloud') == true): ?>style="float:center;"<?php endif; ?>>
          ...
          <form method="post"   action="<?php echo U('Essay/upsave',array('id'=>$_GET['id']));?>"  enctype="multipart/form-data" >
	...
	art.dialog.open('<?php echo U('Map/setLatLng',array('token'=>$token,'id'=>$id));?>',{lock:false,title:'设置经纬度',width:600,height:400,yesText:'关闭',background: '#000',opacity: 0.87});
        ...
        <td>经度 <input type="text" id="longitude"  name="longitude" size="14" class="px" value="<?php echo ($info["longitude"]); ?>" /> 纬度 <input type="text"  name="latitude" size="14" id="latitude" class="px" value="<?php echo ($info["latitude"]); ?>" /> <a href="###" onclick="setlatlng($('#longitude').val(),$('#latitude').val())">在地图中查看/设置</a>&nbsp;&nbsp;可以根据粉丝发送的地理位置后推送最近的图文（可以不填）</td> 
                ...
                <input value="2" name="texttype" type="hidden">
                  <TH></TH>
                  <TD><input type="submit" value="保存" name="sbmt" class="btnGreen left">　<a href="<?php echo U('Essay/index');?>"  class="btnGray vm">取消</a></TD>
                </TR>
              </TBODY>
            </TABLE>
